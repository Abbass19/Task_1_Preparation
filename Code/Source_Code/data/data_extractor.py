...
def data_extractor(asset, cols=['DCP', 'DNCP', 'OPCP', 'HPCP', 'LPCP', 'CPCP', 'ACPCP', 'VTCP', 'MPN5P']):
    ...
    asset_script = f'''
        SELECT *
        FROM "ASSET_{asset}".features_targets_input_data_view
        WHERE "period_type_name" = 'Day' AND "environment_PK" = 8
        ORDER BY "DCP_date_current_period" ASC
    '''
...
df = data_extractor('TSLA', ['DCP', 'DNCP', 'OPCP', 'HPCP', 'LPCP', 'CPCP', 'ACPCP', 'VTCP', 'MPN5P'])



print("We now have the access to this dataset")
